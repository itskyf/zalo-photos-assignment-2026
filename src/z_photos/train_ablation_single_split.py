...
mlflow.set_experiment("Stage4_Ablation_Single_Split")
...
def evaluate_on_clean_set(model, device):
    ...
    return f1_score(y_true, y_pred, average="macro"), balanced_accuracy_score(
        y_true, y_pred
    )
...
def main():
    ...
    for fold in range(5):
        print(f"\n=== Simulating Single Split (Hold out Fold {fold}) ===")
        ...
        args = TrainingArguments(
            output_dir=f"./checkpoints/ablation_single_split_fold_{fold}",
            eval_strategy="epoch",
            save_strategy="epoch",
            learning_rate=1e-3,
            ...
            report_to="none",  # Don't clutter MLflow with 5 fold runs, we'll log summary manually
        )
        ...
        # Evaluate the BEST model from this single split on the actual clean test set
        test_f1, test_bacc = evaluate_on_clean_set(model, device)
        test_f1s.append(test_f1)
        test_baccs.append(test_bacc)
        print(f"-> Test Macro F1 for Split {fold}: {test_f1:.4f}")

    print("\n=== Single Split Variance Results ===")
    print(f"Test Macro F1 across 5 different splits: {[round(x, 4) for x in test_f1s]}")
    mean_f1 = np.mean(test_f1s)
    std_f1 = np.std(test_f1s)
    min_f1 = np.min(test_f1s)
    max_f1 = np.max(test_f1s)

    print(f"Mean: {mean_f1:.4f}")
    print(f"Std Dev: {std_f1:.4f}")
    print(f"Min: {min_f1:.4f}")
    print(f"Max: {max_f1:.4f}")
    print(f"Spread (Max - Min): {max_f1 - min_f1:.4f}")

    with mlflow.start_run(run_name=f"ablation-single-split-variance-{run_ts}"):
        mlflow.log_metric("test_f1_mean", mean_f1)
        mlflow.log_metric("test_f1_std", std_f1)
        mlflow.log_metric("test_f1_min", min_f1)
        mlflow.log_metric("test_f1_max", max_f1)
        mlflow.log_metric("test_f1_spread", max_f1 - min_f1)
        for i, f1 in enumerate(test_f1s):
            mlflow.log_metric(f"test_f1_split_{i}", f1)
