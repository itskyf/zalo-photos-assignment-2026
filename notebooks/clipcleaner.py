...
def get_score(prediction, labels, mode="celoss"):
    """
    :param prediction: we can take the prediction from LR or KNN,
        or utilize the CLIP text probing similarity directly
    :param labels:
    ...
    if mode == "celoss":
        ...
        for id in id_by_label:
            score[id] = (score[id] - score[id].min()) / (
                score[id].max() - score[id].min()
            )
...
def combined_selection(args):
    ...
    if args.model == "small":
        ...
        clip_model, preprocess = clip.load("ViT-L/14@336px")
    clip_data = get_specific_dataset(
        args,
        preprocess,
        mode="train",
    ...
    clip_loader = torch.utils.data.DataLoader(
        clip_data, batch_size=1024, shuffle=False, num_workers=8
    )
    ...
    with torch.no_grad():
        for _i, [data, _label, _index] in enumerate(clip_loader):
            # print(i)
    ...
    all_features = functional.normalize(all_features, dim=1)  # N x d
    del clip_loader
    ...
    text_tokens = [
        [
            clip.tokenize(
                f"a photo of a {class_names[i][0].lower()} {j}" + suffix
            ).cuda()
            if isinstance(class_names[0], list)
            else clip.tokenize(
                f"a photo of a {class_names[i].lower()} {j}" + suffix
            ).cuda()
            for j in detailed_features[i]
        ]
        for i in range(num_classes)
    ]
    text_features = [
        [
            clip_model.encode_text(text_token_i).float().detach()
            for text_token_i in text_token
        ]
        for text_token in text_tokens
    ]  # K x Num of prompts x d
    similarity = torch.zeros(len(all_features), num_classes)  # N x K
    with torch.no_grad():
        for i in range(num_classes):
            text_features_i = torch.cat(
                text_features[i], dim=0
            )  # concatenate multiple prompts: Num of prompts x d
            text_features_i = functional.normalize(text_features_i, dim=1)
            # sample to prompt similarity: N x d ,
            # Num of prompts x d -> N x Num of prompts
            similarity_i1 = torch.einsum("ac, bc->ab", all_features, text_features_i)
            # taking summation over different prompts for class i
            # N x Num of prompts -> N x 1
            similarity[:, i] = torch.exp(similarity_i1 / 0.07).sum(1)
    ...
    clip_similarityprob_consistency = get_score(
        prediction_zero, all_labels, "consistency"
    )
    ...
    # %% save scores
    method_score = [
        clip_similarityprob_loss,
        clip_similarityprob_consistency,
        clip_visuallr_loss,
        clip_visuallr_consistency,
    ]

    method_name = [
        "zeroshot_perclassgmm",
        "zeroshot_consistency",
        "visuallr_perclassgmm",
        "visuallr_consistency",
    ]
    ...
    for i, score in enumerate(method_score):
        ...
        for k in range(num_classes):  # per-class sample selection
            if types[i] == "loss":
                ...
                prob = gmm.predict_proba(score[id_by_label[k]].reshape(-1, 1))[
                    :, gmm.means_.argmax()
                ]
                clean_id = id_by_label[k][np.where(prob >= args.theta_gmm)[0]]
            else:
                clean_id = id_by_label[k][
                    np.where(score[id_by_label[k]] >= args.theta_cons)[0]
                ]
            clean_id_all.append(clean_id)
        clean_id_all = np.concatenate(clean_id_all)
        per_class = np.array(
            [np.sum(all_labels[clean_id_all] == i) for i in range(num_classes)]
        )
    ...
    num_by_class = np.array(
        [np.sum(all_labels[select] == i) for i in range(num_classes)]
    )
    print(num_by_class)
    # at least K samples should be clean!
    # To avoid too small selection for specific class!
